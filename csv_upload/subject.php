...
$query_Recordset1 = "SELECT DISTINCT `Event Title` FROM calender_logs  GROUP BY `Event Title` ORDER BY `Event Title` DESC";
...
$x=$_POST['name'];
mysqli_select_db($local,$database_local);
$query_Recordset2 = "SELECT DISTINCT `Event Title` FROM calender_logs where `Event Title` LIKE  '%$x%%' ";
$query_limit_Recordset2 = sprintf("%s LIMIT %d, %d", $query_Recordset2, $startRow_Recordset2, $maxRows_Recordset2);
$Recordset2 = mysqli_query($local,$query_limit_Recordset2) or die(mysql_error());
$row_Recordset2 = mysqli_fetch_assoc($Recordset2);
echo $row_Recordset2['Event Title'];
header('Location:sub_det_se.php?id='.$row_Recordset2['Event Title']);
ob_end_flush();
?>
